...
impl FunctionCohesionAnalyzer {
    ...
    pub fn analyze(&self, result: &AnalysisResult) -> Result<Vec<FunctionPlacement>> {
        ...
        let file_layers = build_file_layers(&functions);
...
fn build_file_layers(functions: &[FunctionInfo]) -> HashMap<String, String> {
...
fn compute_cohesion_score(
    ...
    for (callee_idx, count) in outgoing {
        ...
        if layer_adheres(func, callee, file_layers) {
            layer_ok += count;
        }
    }
